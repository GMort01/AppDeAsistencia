...
export default function Index() {
  // Estado para controlar qué vista mostrar (null, 'profesor', 'estudiante')
  const [role, setRole] = useState<string | null>(null);
  const [user, setUser] = useState<any>(null);
  const [modoRegistro, setModoRegistro] = useState(false);

  // Función unificada para cerrar sesión y volver al inicio
  const handleLogout = () => {
  ...
  // 1. Lógica para el flujo de Profesor
  if (role === 'profesor') {
    if (!user) {
      return (
        <LoginProfesor 
  ...
  // 2. Lógica para el flujo de Estudiante
  if (role === 'estudiante') {
    if (!user) {
      return (
        <LoginEstudiante 
    ...
    return (
      <EstudianteView 
        user={user} 
        onLogout={handleLogout} // Conexión vital para el botón de salir
        onBack={() => setRole(null)} // Por si quieres permitir volver atrás sin desloguear
      />
    );
  }

  // 3. Lógica para el Registro
  if (modoRegistro) {
    return <RegistroEstudiante onBack={() => setModoRegistro(false)} />;
  }

  // 4. Pantalla inicial (Selección de Rol)
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity
        ...
        <Text style={styles.registerText}>¿Nuevo? Regístrate</Text>
      </TouchableOpacity>
